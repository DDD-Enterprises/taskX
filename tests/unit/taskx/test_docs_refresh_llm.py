...
def test_check_mode_returns_exit_1_on_drift(tmp_path: Path, monkeypatch) -> None:
    ...
    agents = agents.replace("Generated by: taskx docs refresh-llm", "Generated by: drift", 1)
    ...
    report = json.loads(
        (repo_root / "out" / "taskx_docs_refresh_llm" / "DOCS_REFRESH_LLM_REPORT.json").read_text(
            encoding="utf-8"
        )
    )
...
def test_router_summary_source_selection_and_ladder_order(tmp_path: Path) -> None:
    ...
    assert "### Assisted Routing (taskx route)" in no_repo_availability["autogen_block"]

    availability_path = repo_root / ".taskx" / "runtime" / "availability.yaml"
...
def test_command_tree_contains_nested_leaf_depth(tmp_path: Path) -> None:
    ...
    assert "- taskx docs" in block
    assert "  - taskx docs refresh-llm" in block
    assert "- taskx project" in block
    assert "    - taskx project shell init" in block
    assert "  - taskx route plan" in block
