"""Project readiness doctor for dopeTask/ChatX directive workflows."""
        ...
        checks.append(
            {
                "id": "sentinel_integrity",
                "status": "pass",
                "message": "dopeTask/ChatX sentinel blocks are present and well-formed",
...
def _detect_mode(per_file_status: dict[str, dict[str, str]]) -> str:
    ...
    for states in per_file_status.values():
        dopetask_state = states.get("dopetask")
        chatx_state = states.get("chatx")
        if dopetask_state not in {"enabled", "disabled"}:
            return "unknown"
        if chatx_state not in {"enabled", "disabled"}:
            return "unknown"
        observed.add((dopetask_state == "enabled", chatx_state == "enabled"))
...
def _find_drift_files(per_file_status: dict[str, dict[str, str]]) -> list[str]:
    ...
    for filename, states in per_file_status.items():
        signature = (states.get("dopetask", "unknown"), states.get("chatx", "unknown"))
...
def _render_supervisor_prompt(mode: str) -> str:
    ...
    dopetask_text = read_pack_text("dopetask") if mode_config["dopetask"] else DISABLED_TEXT
    chatx_text = read_pack_text("chatx") if mode_config["chatx"] else DISABLED_TEXT

    lines = [
        "# SUPERVISOR_PRIMING_PROMPT",
        "",
        "Generated by dopetask project doctor/mode workflows.",
        f"mode: {mode}",
        "",
        "## DOPETASK",
        dopetask_text,
...
def _render_doctor_markdown(project_dir: Path, report: dict[str, Any]) -> str:
    ...
    if per_file_status:
        for filename in sorted(per_file_status):
            ...
            lines.append(
                f"- {filename}: dopetask={states.get('dopetask', 'unknown')} chatx={states.get('chatx', 'unknown')}"
            )
    else:
        lines.append("- (none)")
